...
export const useZone = (cameraId: string) => {
  ...
  const updateZona = (id: string, changes: Partial<Zone>) => {
    setZonas((prev) =>
      prev.map((z) => (z.zone_id === id ? { ...z, ...changes } : z)),
    );
  };
  ...
  const clearAll = async () => {
    try {
      ...
      const savedZonas = zonas.filter(
        (z) => z.zone_id && !z.zone_id.startsWith("TEMP-"),
      );
  ...
  const saveAll = async () => {
    ...
    try {
      const payload: Partial<Zone>[] = zonas.map((z) => {
        ...
        return zone_id && !zone_id.startsWith("TEMP-")
          ? { zone_id, ...rest }
          : rest;
      });
