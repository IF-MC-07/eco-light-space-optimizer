...
Dibuat otomatis menggunakan MarkItDown.
...
def open_capture(cam_source, timeout_ms=5000):
    if isinstance(cam_source, int) and os.name == 'nt':
        ...
        try:
            cap.set(cv2.CAP_PROP_OPEN_TIMEOUT_MSEC, timeout_ms)   # 5 detik timeout koneksi
            cap.set(cv2.CAP_PROP_READ_TIMEOUT_MSEC, timeout_ms)   # 5 detik timeout baca frame
        except Exception:
            pass
    return cap
...
def open_capture(cam_source, timeout_ms=5000):
    if isinstance(cam_source, int) and os.name == 'nt':
        ...
        backend = cv2.CAP_FFMPEG if isinstance(cam_source, str) else 0
